...
document.addEventListener("DOMContentLoaded", function () {
  const targetDate = new Date(2026, 4, 1, 13, 0, 0).getTime();
...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  const API_BASE_URL = "https://backend-boda.fly.dev/api";
  ...
  if (token) {
    try {
      ...
      if (
        guestData["TicketsConfirmados"] > 0 ||
        guestData["RespondioFormulario"]
      ) {
        if (form) {
          form.innerHTML =
            "<h3 style='text-align: center; color: #333;'>Ya has respondido a esta invitación. ¡Gracias!</h3>";
        }
        return;
      }
      ...
      if (formAsistencia) {
        ...
        for (let i = totalTickets; i > 0; i--) {
          ...
          if (i === totalTickets) {
            opt.innerText =
              totalTickets > 1 ? `Asistiremos los ${i}` : "Asistiré solo";
          } else if (i === 1) {
  ...
  if (form) {
    form.addEventListener("submit", async function (e) {
      ...
      try {
        ...
        form.innerHTML =
          "<h3 style='text-align: center; color: #333;'>¡Gracias por confirmar tu asistencia! Hemos registrado tus datos.</h3>";
      } catch (error) {
